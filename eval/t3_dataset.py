...
class T3DataSet(Dataset):
    ...
    def load_data(self, json_path, percent):
        ...
        content['data_root'] = osp.join(osp.dirname(json_path), 'imgs')
        ...
        for gt in content['data_list']:
            ...
            # add valid info
            info['valid_img'] = self.isvalid_img(gt['annotations'])
    ...
    def __getitem__(self, item):
        ...
        # img
        if self.use_filter and not cur_item['valid_img']:
            target = np.array(Image.open('assets/orange_image.png').convert('RGB'))
        else:
